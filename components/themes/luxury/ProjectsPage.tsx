    ...
    <div className="min-h-screen bg-[#080808]" dir="rtl">
      ...
      <nav className="px-8 py-5 flex items-center justify-between max-w-7xl mx-auto">
        ...
        <div className="flex gap-8 text-[11px] text-white/30 tracking-[0.25em] uppercase">
          ...
          <span className="text-[#c9a84c]">{sc.portfolioLabel}</span>
      ...
      <div className="max-w-7xl mx-auto px-8 py-16">
        ...
        <div className="mb-16">
          <div className="flex items-center gap-4 mb-6">
            ...
            <span className="text-[#c9a84c] text-[10px] tracking-[0.4em] uppercase">{sc.featuredLabel}</span>
          </div>
          <h1 className="text-6xl font-light text-white">{sc.portfolioLabel}</h1>
          <p className="text-white/20 text-sm mt-2">{projects.length} {sc.portfolioItemLabel}</p>
        </div>
          ...
          {filtered.map(p => (
            <Link key={p.id} href={`/${tenant.slug}/projects/${p.id}`}
              ...
              <div className="relative w-48 md:w-80 h-full flex-shrink-0 overflow-hidden">
                ...
                {sc.extraFields.status && p.status && (
                  <div className={`absolute top-2 right-2 px-2 py-0.5 text-[10px] font-bold ${
                    p.status === 'متاح' ? 'bg-green-800 text-green-200' :
                    p.status === 'مباع' ? 'bg-red-900 text-red-200' :
              ...
              <div className="flex-1 px-8">
                ...
                <div className="flex flex-wrap gap-4 mt-2 text-white/20 text-xs">
                  {sc.extraFields.price && p.price && <span className="text-[#c9a84c]">{p.price}</span>}
        ...
        {filtered.length === 0 && (
          <p className="text-center text-white/20 py-16">لا توجد {sc.portfolioItemLabelPlural} في هذه الفئة</p>
        )}
      </div>

...
| الحقل | الاستخدام | مثال |
|-------|-----------|------|
| `primary` | الخلفية الرئيسية الداكنة (Nav، Hero، Footer) | `#0f0f0f` |
| `secondary` | خلفية ثانوية أقل عتمة (بطاقات الخدمات) | `#1a1a1a` |
| `accent` | لون التمييز (أزرار، أيقونات، خطوط تزيينية) | `#c9a84c` |
| `background` | خلفية الأقسام الفاتحة | `#ffffff` |
| `text` | لون النص الرئيسي | `#111111` |
| `textLight` | نص ثانوي / تعليقات / تفاصيل | `#888888` |
...
| الحقل | الاستخدام |
|-------|-----------|
| `heading` | العناوين الكبيرة (H1، H2، اسم المكتب) |
| `body` | نص الموقع العام والتفاصيل |
...
> **ملاحظة:** الخطوط تُحمَّل تلقائياً من Google Fonts باسمها.
...
| القيمة | الوصف |
|--------|-------|
| `"fullscreen"` | يغطي الشاشة كاملاً مع صورة خلفية وتدرج لوني. الافتراضي. |
...
| `"lg"` | حواف دائرية — `16px` |
| `"full"` | دائري تماماً (للأزرار والعناصر الصغيرة) |

### `spacing` — المسافات العمودية بين الأقسام

| القيمة | التأثير |
|--------|---------|
| `"compact"` | مسافات صغيرة — `py-12` (48px) |
| `"normal"` | مسافات متوازنة — `py-20` (80px) |
| `"spacious"` | مسافات واسعة فاخرة — `py-32` (128px) |

### `sections` — ترتيب أقسام الصفحة

هذا المصفوفة **تحدد ما يظهر وبأي ترتيب** في الصفحة الرئيسية:
...
## ٥. شبكة المشاريع والخدمات — `projectsGrid`
...
| القيمة | عدد الأعمدة على الشاشة الكبيرة |
|--------|--------------------------------|
| `2` | عمودان — مناسب للصور الأفقية |
| `3` | ثلاثة أعمدة — الأكثر شيوعاً |
| `4` | أربعة أعمدة — للمشاريع الكثيرة |

> على الجوال يتحول تلقائياً إلى عمود أو عمودين

### `style` — أسلوب عرض المشاريع

| القيمة | الوصف |
|--------|-------|
| `"grid"` | شبكة منتظمة — كل الصور بنفس الحجم (مربعات) |
| `"masonry"` | تدرجي كـ Pinterest — صور بأحجام مختلفة |
| `"list"` | قائمة أفقية — صورة صغيرة + اسم المشروع |
...
7. انتظر رسالة النجاح — القالب متاح فوراً
...
- ❌ لا تستخدم خطوط لا تدعم العربية لـ `body`
...
### الأقسام
- 🚀 **أقل أقسام = موقع أسرع وأنظف**
- ✅ الحد الأدنى الموصى به: `["hero", "projects", "footer"]`
- ⭐ الكامل المثالي: `["hero", "about", "services", "projects", "features", "cta", "footer"]`

### المعاينة
- 📸 صورة `preview.jpg` مهمة جداً — هي ما يراه أصحاب المكاتب قبل الاختيار
...
هذه العناصر ثابتة ومدمجة في المنصة:
- نصوص الموقع (اسم المكتب، النبذة، المشاريع) — تأتي من داشبورد المكتب
- الخدمات والمميزات — يضيفها المكتب من داشبورده
- شعار المكتب وصورة الغلاف — يرفعها المكتب
- روابط التواصل الاجتماعي

---

*آخر تحديث: 2026-05-24 | إصدار 1.0*
